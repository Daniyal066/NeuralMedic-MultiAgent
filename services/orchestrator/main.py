        ...
        async with pool.acquire() as conn:
            ...
            if url:
                ...
                async with httpx.AsyncClient() as client:
                    try:
                        headers = {"X-API-Key": os.getenv("INTERNAL_API_KEY", "default_internal_secret_key")}
                        response = await client.post(f"{url}/analyze/{endpoint}/{session_id}", headers=headers, timeout=120.0)
